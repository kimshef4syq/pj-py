...
logger = Logger.GetConsoleLogger("KineManipulability")
...
class Manipulability(object):
    def __init__(self):
        ...
        self.m_manipulability_stats = None
        self.m_max_manipulability = None
    ...
    def Init(self, manipulator, kin_solver):
        self.manipulator = manipulator
        self.m_robot_model = manipulator.GetRobotModel()
        # default path
        self.m_robot_name = self.m_robot_model.GetName()
        self.m_manip_name = self.manipulator.GetName()
        self.m_file_name = GetDataPath() + "Database/Manipulability/{0}_{1}_{2}_{3}.h5".format(
            self.m_robot_name, self.m_manip_name, self.m_xyz_delta, self.m_quat_delta)

        self.m_kin_solver = kin_solver
        self.m_max_manipulability = 0
        self.m_is_init = True
        return True
    ...
    def SaveHDF5(self):
        if not self.m_is_init:
            logger.Error("KineManipulability has not been initialized")
            return False
        ...
        f['manipulability_stats'] = self.m_manipulability_stats
        f['max_manipulability'] = self.m_max_manipulability
    ...
    def LoadHDF5(self):
        ...
        self.m_manipulability_stats = f['manipulability_stats']
        self.m_max_manipulability = f['max_manipulability'].value
    ...
    def GenerateManipulabilityData(self, sample_type=0, axis=[0, 0, 1], posture=[0, 0, 0, 1]):
        """
        Generate manipulability data.
        ...
        for i, ind in enumerate(inside_inds):
            ...
            for quat in qarray:
                ...
                if kin_solver.HasAnalyticalSolution():
                    res, ik_report = kin_solver.GetPositionIK(pose)
                else:
                    res, ik_report = kin_solver.SearchAllPositionIK(pose)

                if res == RVSReturn_Success:
                    for q in ik_report.ik_vector:
                        tmp_manipulability = kc.ExtendedManipulability(
                            q, ManipubilityType_MinMaxRatio)
                        # tmp_manipulability = kc.Manipulability(q, ManipubilityType_MinMaxRatio)
                        if tmp_manipulability > best_manipulability:
                            best_manipulability = tmp_manipulability

            if best_manipulability > self.m_max_manipulability:
                self.m_max_manipulability = best_manipulability

            self.m_manipulability_stats.append(
                np.r_[trans, best_manipulability])
        ...
        self.m_manipulability_stats = np.array(self.m_manipulability_stats)

    def ShowManipulabilityData(self, show_type=0, plane=None, distance=0, env=None):
        """
        Show manipulability data.
        ...
        plane: the coefficients [x0,y0,z0,a,b,c] of the plane that will display, (x0,y0,z0)  the point in the plane, (a,b,c) represents the normalvector,
               subject to a(x-x0) + b(y-y0) + c(z-z0) = 0. When plane is None, it will show all the worksapce manipulability, or it will show the manipulability in the plane. 
        """
        if not env == None:
        ...
        if show_type == 0:  # Todo
            if plane is not None:  # to show workspace in plane
                ...
                if np.linalg.norm(n) < 1e-2:
                    ...
                    return
                for stats in self.m_manipulability_stats:
                    point = stats[0:3]
                    point0 = plane[0:3]
                    n = plane[3:6]
                    ...
                    if dist > max(radius, distance):
                        continue
                    ratio = stats[-1] / self.m_max_manipulability
                    if stats[-1] > 1e-4:

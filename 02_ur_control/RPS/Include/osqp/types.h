 ...
 *  The structure is used internally to store matrices in the triplet form as
 * well, but the API requires that the matrices are in the CSC format.
 */
typedef struct {
    ...
    c_int
        *p; ///< column pointers (size n+1); col indices (size nzmax) start from
            ///< 0 when using triplet format (direct KKT matrix formation)
...
typedef struct {
  ...
  csc *P; ///< the upper triangular part of the quadratic cost matrix P in csc
          ///< format (size n x n).
...
# if EMBEDDED != 1
  ...
  c_int adaptive_rho_interval; ///< number of iterations between rho
                               ///< adaptations; if 0, then it is automatic
  c_float adaptive_rho_tolerance; ///< tolerance X for adapting rho. The new rho
                                  ...
                                  ///< new factorization.
#  ifdef PROFILING
  c_float adaptive_rho_fraction; ///< interval for adapting rho (fraction of the
                                 ///< setup time)
#  endif // Profiling
# endif // EMBEDDED != 1
...
# ifndef EMBEDDED
  ...
  c_int
      polish_refine_iter; ///< number of iterative refinement steps in polishing

  c_int verbose;                         ///< boolean, write out progress
# endif // ifndef EMBEDDED

  c_int scaled_termination; ///< boolean, use scaled termination criteria
  c_int check_termination; ///< integer, check termination interval; if 0, then
                           ///< termination checking is disabled
  c_int warm_start;                      ///< boolean, warm start

# ifdef PROFILING
  c_float time_limit; ///< maximum number of seconds allowed to solve the
                      ///< problem; if 0, then disabled
# endif // ifdef PROFILING
} OSQPSettings;
...
#ifndef EMBEDDED
    void (*free)(LinSysSolver *self); ///< free linear system solver (only in
                                      ///< desktop version)
#endif // ifndef EMBEDDED

...
*******************/
#define OSQP_VERSION                                                           \
    ("0.6.2") /* string literals automatically null-terminated                 \
               */
...
#define RHO_TOL                                                                \
    (1e-04) ///< tolerance for detecting if an inequality is set to equality
...
#define ADAPTIVE_RHO_FRACTION                                                  \
    (0.4) ///< fraction of setup time after which we update rho
#define ADAPTIVE_RHO_MULTIPLE_TERMINATION                                      \
    (4) ///< multiple of check_termination after which we update rho (if
        ///< PROFILING disabled)
#define ADAPTIVE_RHO_FIXED                                                     \
    (100) ///< number of iterations after which we update rho if
          ///< termination_check  and PROFILING are disabled
#define ADAPTIVE_RHO_TOLERANCE                                                 \
    (5) ///< tolerance for adopting new rho; minimum ratio between new rho and
        ///< the current one
# endif // if EMBEDDED != 1

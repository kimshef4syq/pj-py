...
/* Copied from
 * fullfile(matlabroot,'extern','include','coder','coder_array','coder_array_rtw.h')
 */
...
public:
    ...
    data_ptr() : data_(NULL), size_(0), capacity_(0), owner_(false) {}
...
public:
    ...
    T &operator[](SZ _index) { return data_[_index]; }
    const T &operator[](SZ _index) const { return data_[_index]; }
    ...
    void set_owner(bool _b) { owner_ = _b; }
...
{
public:
    array_iterator() : arr_(NULL), i_(0) {}
    array_iterator(const array_iterator<T> &other)
        : arr_(other.arr_), i_(other.i_)
    {
    }
    ~array_iterator() {}
    typename T::value_type &operator*() const { return (*arr_)[i_]; }
    typename T::value_type *operator->() const { return &(*arr_)[i_]; }
...
{
public:
    const_array_iterator() : arr_(NULL), i_(0) {}
    const_array_iterator(const const_array_iterator<T> &other)
        : arr_(other.arr_), i_(other.i_)
    {
    }
    ~const_array_iterator() {}
    const typename T::value_type &operator*() const { return (*arr_)[i_]; }
    const typename T::value_type *operator->() const { return &(*arr_)[i_]; }
...
public:
    ...
    static SZ compute(const SZ _size[], const SZ _indices[])
    {
        ...
        return weight * _indices[I - 1]
               + index_nd<I - 1>::compute(_size, _indices);
    }
};
...
public:
    ...
    array_base() { (void)::memset(size_, 0, sizeof(SZ) * N); }
    ...
    void set(T *_data, SZ _n1, SZ _n2, SZ _n3, SZ _n4, SZ _n5, SZ _n6, SZ _n7,
             SZ _n8, SZ _n9, SZ _n10)
    {
        coder::detail::match_dimensions<N == 10>::check();
        data_.set(_data,
                  _n1 * _n2 * _n3 * _n4 * _n5 * _n6 * _n7 * _n8 * _n9 * _n10);
        size_[0] = _n1;
        size_[1] = _n2;
        size_[2] = _n3;
        size_[3] = _n4;
    ...
    bool is_owner() const { return data_.is_owner(); }

    void set_owner(bool b) { data_.set_owner(b); }

    SZ capacity() const { return data_.capacity(); }
    ...
    array_base<T, SZ, 6> reshape(SZ _n1, SZ _n2, SZ _n3, SZ _n4, SZ _n5,
                                 SZ _n6) const
    {
    ...
    array_base<T, SZ, 9> reshape(SZ _n1, SZ _n2, SZ _n3, SZ _n4, SZ _n5, SZ _n6,
                                 SZ _n7, SZ _n8, SZ _n9) const
    {
        const SZ ns[] = {_n1, _n2, _n3, _n4, _n5, _n6, _n7, _n8, _n9};
        return reshape_n(ns);
    }

    array_base<T, SZ, 10> reshape(SZ _n1, SZ _n2, SZ _n3, SZ _n4, SZ _n5,
                                  SZ _n6, SZ _n7, SZ _n8, SZ _n9, SZ _n10) const
    {
    ...
    const SZ *size() const { return &size_[0]; }

    SZ size(SZ _index) const { return size_[_index]; }

    SZ numel() const { return coder::detail::numel<N>::compute(size_); }
    ...
    SZ index(SZ _n1, SZ _n2, SZ _n3, SZ _n4, SZ _n5, SZ _n6, SZ _n7,
             SZ _n8) const
    {
        coder::detail::match_dimensions<N == 8>::check();
    ...
    const T &at(SZ _i1, SZ _i2, SZ _i3, SZ _i4, SZ _i5, SZ _i6, SZ _i7,
                SZ _i8) const
    {
        coder::detail::match_dimensions<N == 8>::check();
        return data_[index(_i1, _i2, _i3, _i4, _i5, _i6, _i7, _i8)];
...
public:
    array() : Base() {}
    array(const array<T, N> &_other) : Base(_other) {}
    array(const Base &_other) : Base(_other) {}
    array(T *_data, const SizeType *_sz) : Base(_data, _sz) {}
};
...
public:
    array() : array_base() {}
    array(const array<char_T, 2> &_other) : Base(_other) {}
    array(const Base &_other) : Base(_other) {}

    array(const std::string &_str) { operator=(_str); }

    array(const char_T *_str) { operator=(_str); }
    ...
    {
        return std::string(static_cast<const char *>(&(*this)[0]),
...
public:
    array() : Base() {}
    array(const array<T, 2> &_other) : Base(_other) {}
    array(const Base &_other) : Base(_other) {}
    array(const std::vector<T> &_vec) { operator=(_vec); }
...
public:
    array() : Base() {}
    array(const array<T, 1> &_other) : Base(_other) {}
    array(const Base &_other) : Base(_other) {}
    array(const std::vector<T> &_vec) { operator=(_vec); }

...
class PathSegmentCircleCircumscribed : public PathSegmentBase<LieGroup>
{
public:
    RVS_DECLARE_PTR_MEMBER(PathSegmentCircleCircumscribed<LieGroup>)
    DEFINE_SPACE_TYPES(LieGroup)
    /**
     * @brief Construct a new PathSegmentCircleCircumscribed
     *
     ...
     * kept as 1.0
     * @param is_full_circle whether the cirlce is closed or open
     */
    PathSegmentCircleCircumscribed(const std::array<LieGroup, 3> &waypoints,
                                   double s0 = 0.0,
    ...
    {
        return std::make_shared<PathSegmentCircleCircumscribed>(*this);
    }
...
private:
    /// @brief Initialize a circumscribed circle segment
    bool _InitCircumscribedCircleSegment(bool is_full_circle = false);

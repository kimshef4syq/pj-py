...
{
public:
    explicit TrajQPSolver(const CVecXd &ts, double poly_order);

    virtual ~TrajQPSolver() {}

    /** @brief set the quadratic cost matrix of the QP problem*/
    virtual bool SetQuadCostMat(size_t deriv_order) = 0;
    ...
    virtual bool Solve(CVecXd &poly_coeffs, bool verbose = false);
